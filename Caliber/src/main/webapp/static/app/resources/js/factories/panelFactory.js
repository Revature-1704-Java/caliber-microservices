...
angular.module('api').factory('panelFactory', function($log, $http) {
	...
	panel.findAllPanels = function () {
		return $http({
			url: 'localhost:8081/panel/panel/all',
			method: 'GET'
	...
	panel.reportPanelTable = function (batchId) {
		return $http({
			url: 'localhost:8081/panel/all/reports/batch/' + batchId + '/panel-batch-all-trainees',
			method: 'GET'
		}).then(function (response) {
	...
	panel.getPanelById = function (panelId) {
		return $http({
			url: 'localhost:8081/panel/panel/' + panelId,
			method: 'GET'
		}).then(function (response) {
	...
	panel.reportTraineePanels = function (traineeId) {
		return $http({
			url : 'localhost:8081/panel/panel/trainee/' + traineeId,
			method : 'GET'
		}).then(function (response) {
	...
	panel.reportAllRepanels = function () {
		return $http({
			url : 'localhost:8081/panel/panel/repanel/all',
			method : 'GET'
		}).then(function (response) {
	...
	panel.updatePanel = function (panelObj) {
		return $http({
			url : 'localhost:8081/panel/panel/update',
			method : 'PUT',
			data : panelObj
		}).then(function (response) {
	...
	panel.createPanel = function (panelObj) {
		return $http({
			url : 'localhost:8081/panel/panel/create',
			method : 'POST',
			data : panelObj
		}).then(function (response) {
	...
	panel.deletePanel = function (panelId) {
		return $http({
			url : 'localhost:8081/panel/panel/delete/' + panelId,
			method : 'DELETE'
		}).then(function (response) {
	...
	panel.getRecentRepanels = function () {
		return $http({
			url : 'localhost:8081/panel/panel/repanel/recent',
			method : 'GET'
		}).then(function (response) {
